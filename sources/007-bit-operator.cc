...
// 0. Check ODD or EVEN?
template <typename T>
constexpr T is_odd(T digit) noexcept(noexcept(T(digit))) {
    return 1U & digit;
}
...
template <typename T>
constexpr T mult_two(T opnd) noexcept(noexcept(T(opnd))) {
    return opnd << 1U;
}
...
template <typename T>
constexpr T divide_two(T opnd) noexcept(noexcept(T(opnd))) {
    return opnd >> 1U;
}
